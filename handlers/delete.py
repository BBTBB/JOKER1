...
⏺꒐ تم طرده لتجاوز عدد الرسائل المحدده {} في الوقت المحدد {}""".format(BY,Max_msg,Time_ck),"parse_mode":"html"})
  ...
  if re.findall("[Hh][Tt][Tt][Pp][Ss]:/|[Hh][Tt][Tt][Pp]://|.[Ii][Rr]|.[Cc][Oo][Mm]|.[Oo][Rr][Gg]|.[Ii][Nn][Ff][Oo]|[Ww][Ww][Ww]|.[Tt][Kk]|.[Mm][Ee]", text) or re.findall('@', text) or re.findall('#', text) or message.via_bot or message.reply_markup or message.sticker or message.animation or message.audio or message.voice or message.video or message.document or message.photo or message.contact or message.forward_date or message.video_note:
